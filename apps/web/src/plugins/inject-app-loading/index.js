...
export const injectAppLoadingPlugin = async () => {
  ...
  const loadingDiv = loadingHtml.match(/<div id="app-loading"[\s\S]*<\/div>/)[0]

  return {
    name: 'inject-app-loading',
    transformIndexHtml(html) {
      return html.replace(/(<\/head>)/, `${loadingStyle}$1`).replace(/(<body>)/, `$1${loadingDiv}`)
    },
  }
}
